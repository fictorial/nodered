...
Dispatch.prototype.addRequestsForClient = function (client) {
  ...
  if (clientContext.requestQueue.length == 0)
    clientContext.requestQueue = client.requestQueue;
  else
    clientContext.requestQueue = clientContext.requestQueue.concat(client.requestQueue);
...
Dispatch.prototype.dispatchOne = function (clientID) {
  ...
  if (DEBUG) logger.debug("DISPATCH/COMMAND " + cmd);
