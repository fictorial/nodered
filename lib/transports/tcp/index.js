...
function TCPTransport(controller, commandLineOptions, netModule, logName) {
  this.controller = controller;
  this.dispatch = new Dispatch(controller);
  this.logName = logName || 'TCP';

  var self = this;
  this.server = (netModule || require("net")).createServer(
    function (stream) {
      ...
      stream.addListener("data", 
        function (chunk) {
          ...
          if (DEBUG) 
            logger.debug(self.logName + "/RECV  " 
                                      + client.nickname 
                                      + "@" 
          ...
          try {
            ...
            logger.error(self.logName + "/ERROR  " + e);
  ...
  var port = commandLineOptions.port || 8080
    , host = commandLineOptions.host || '127.0.0.1';

  this.server.listen(port, host);
  logger.info(this.logName + "/LISTEN  " 
                           + host 
                           + ":" 
                           + port);
}
...
exports.create = function (controller, commandLineOptions, netModule, logName) {
  return new TCPTransport(controller, commandLineOptions, netModule, logName);
};
...
TCPTransport.prototype.sendReply = function (client, options) {
  if (client.stream && client.stream.writable) {
    ...
    if (DEBUG) 
      logger.debug(this.logName + "/SEND  " 
                                + client.nickname 
                                + "@" 
                                + client.stream.remoteAddress 
                                + " " 
